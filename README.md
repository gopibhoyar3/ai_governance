...
```bash
createdb ai_governance_db
```

If `createdb` is not available, create it using pgAdmin or psql:
...
```bash
cd ai-governance-control-hub-backend
python3 -m venv venv
...
Update `.env` if your PostgreSQL username/password is different:

```text
DATABASE_URL=postgresql+psycopg2://postgres:postgres@localhost:5432/ai_governance_db
```
...
Use either `python create_db.py` or migrations, not both for the first setup.
...
```json
{
  "service": "AI Governance Control Hub Backend",
  "status": "UP"
}
```
